...
#FIXED FUNCTION FOR NEXT STEP
def channel_FIXED(signal):
...
#show image again
img = cv2.imdecode(img,cv2.IMREAD_COLOR) #1 is cv2.IMREAD_COLOR (BGRcolor)
while(True):
...
# ## Next section, we wil send all frames to the channel then calulating average ber 
...
img_back = img_back.reshape(imgx.shape[0], imgx.shape[1], imgx.shape[2])
    #save and show image
cv2.imwrite('img_final.jpg', img_back) 
imgfinal = cv2.imread('img_final.jpg', 1)
while(True):
